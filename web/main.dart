...
String _icon(String name, {String size = '20'}) {
  const paths = <String, String>{
    ...
    'user': '<circle cx="12" cy="8" r="3.5"/><path d="M5 20c.7-3.5 3.2-5.5 7-5.5s6.3 2 7 5.5"/>',
    ...
    'medical': '<path d="M12 20s-7-4.4-7-10a3.7 3.7 0 0 1 6.5-2.4L12 8l.5-.4A3.7 3.7 0 0 1 19 10c0 5.6-7 10-7 10Z"/><path d="M12 7v6m-3-3h6"/>',
    'book': '<path d="M5 4.5A2.5 2.5 0 0 1 7.5 4H20v15H7.5A2.5 2.5 0 0 0 5 21.5v-17Z"/><path d="M5 19.5A2.5 2.5 0 0 1 7.5 17H20"/>',
    'compass': '<circle cx="12" cy="12" r="8"/><path d="m14.8 9.2-1.7 3.9-3.9 1.7 1.7-3.9 3.9-1.7Z"/>',
    'trophy': '<path d="M8 4h8v5a4 4 0 0 1-8 0V4Z"/><path d="M8 6H5v1a4 4 0 0 0 4 4m7-5h3v1a4 4 0 0 1-4 4M12 13v4m-3 3h6"/>',
    'wallet': '<path d="M5 7.5V6a2 2 0 0 1 2-2h11v16H7a2 2 0 0 1-2-2V7.5Z"/><path d="M5 8h14v5H5"/><circle cx="15" cy="10.5" r=".7"/>',
    'file': '<path d="M7 3h7l4 4v14H7a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2Z"/><path d="M14 3v5h5M8.5 13h7m-7 3h7"/>',
    'users': '<circle cx="9" cy="9" r="3"/><path d="M3.5 20c.5-3.2 2.4-5 5.5-5s5 1.8 5.5 5M16 7.5a3 3 0 0 1 0 5.8m1.2 2.1c2 .4 3.2 2 3.5 4.6"/>',
...
void _renderEvents([String filter = '']) {
  ...
  querySelector('#page-content')!.setInnerHtml('''
    ...
    <section class="events-grid">${matching.isEmpty ? '<div class="empty"><div class="empty-icon">${_icon('calendar', size: '42')}</div><h3>No events found</h3><p>Try another search or create a school event.</p></div>' : matching.map(_eventCard).join()}</section>
  ''', treeSanitizer: NodeTreeSanitizer.trusted);
...
void _renderUpload() {
  ...
  querySelector('#page-content')!.setInnerHtml('''
    ...
    ${events.isEmpty ? '<div class="empty"><div class="empty-icon">${_icon('upload', size: '42')}</div><h3>No event available</h3><p>Add an event before submitting proof.</p><button class="btn btn-primary" data-page="events">Go to events</button></div>' : '''<div class="upload-stepper" aria-label="Upload progress"><span class="active"><b>1</b> Choose event</span><span class="${selected == null ? '' : 'active'}"><b>2</b> Add evidence</span><span class="${proofSubmitted ? 'active' : ''}"><b>3</b> Complete</span></div><div class="upload-layout"><section class="panel"><h2>Choose event</h2><div class="event-select">${events.map((event) => '<button class="event-choice ${event['id'] == selectedEventId ? 'selected' : ''}" data-select-event="${_e(event['id'])}"><strong>${_e(event['title'])}</strong><span>${_e(event['date'])} · ${_e(event['requirement'])}</span></button>').join()}</div></section><section class="panel"><h2>Add evidence</h2><label class="dropzone" for="proof-file"><input id="proof-file" type="file" accept="image/*"><span class="dropzone-icon">${_icon('upload', size: '48')}</span><strong>${proofDataUrl == null ? 'Choose a proof image' : 'Proof image selected'}</strong><span>${proofDataUrl == null ? 'JPG, PNG or a photo from your device' : 'Review the preview, then submit it.'}</span>${proofDataUrl == null ? '' : '<img class="preview" src="${_e(proofDataUrl)}" alt="Proof preview">'}</label><button class="btn btn-primary btn-block" id="submit-proof" style="margin-top:16px" ${proofDataUrl == null ? 'disabled' : ''}>Submit attendance proof</button><div id="upload-message"></div></section></div>'''}
  ''', treeSanitizer: NodeTreeSanitizer.trusted);
  _bindPageButtons();
  ...
  querySelector('#submit-proof')?.onClick.listen((_) {
    ...
    proofSubmitted = true;
    _saveCompleted();
    querySelector('#upload-message')?.setInnerHtml(
      '<div class="notice">✓ Proof saved. ${_e(selected['requirement'])} is now cleared.</div>',
      treeSanitizer: NodeTreeSanitizer.trusted,
    );
  });
}
...
void _renderProfile() {
  ...
  querySelector('#page-content')!.setInnerHtml('''
    ...
    <div class="profile-grid"><section class="panel profile-card"><div class="profile-photo">${profile?['photo'] == null ? _e(_initials()) : '<img src="${_e(profile!['photo'])}" alt="Profile photo">'}</div><h2>${_e(profile?['name'])}</h2><p>${_e(profile?['id'])}</p><button class="btn btn-outline btn-block" id="edit-profile">Edit profile</button></section>
    <section class="panel"><h2>Student information</h2><div class="details"><div class="detail"><span>Department</span><strong>${_e(profile?['department'])}</strong></div><div class="detail"><span>Year level</span><strong>${_e(profile?['year'])}</strong></div><div class="detail"><span>Program</span><strong>${_e(profile?['program'])}</strong></div><div class="detail"><span>Student ID</span><strong>${_e(profile?['id'])}</strong></div></div>
      <div class="setting"><div><strong>Display mode</strong><div style="color:var(--muted);font-size:13px">Choose your preferred appearance</div></div><div class="theme-options">${[
    'light',
    'dark',
    'amoled'
